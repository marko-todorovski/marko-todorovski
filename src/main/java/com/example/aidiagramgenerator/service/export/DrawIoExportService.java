...
public class DrawIoExportService {
    ...
    public String convertPlantUmlToDrawIoXml(String plantUmlCode,
        ...
        logger.info("Converting PlantUML {} diagram to Draw.io XML (codeLength={})", domainType, plantUmlCode.length());
        logger.debug("PlantUML code: {}", plantUmlCode);
        ...
        try {
            switch (domainType) {
                case SEQUENCE -> {
                    builder.setLayoutStrategy(DrawIoXmlBuilder.LayoutStrategy.HORIZONTAL);
                    parsePlantUmlSequence(plantUmlCode, builder);
                }
                case CLASS -> {
                    builder.setLayoutStrategy(DrawIoXmlBuilder.LayoutStrategy.HIERARCHICAL);
                    parseClassDiagram(plantUmlCode, builder);
                }
                case ER -> {
                    builder.setLayoutStrategy(DrawIoXmlBuilder.LayoutStrategy.GRID);
                    builder.setNodesPerRow(3);
                    parsePlantUmlEr(plantUmlCode, builder);
                }
                case COMPONENT, DEPLOYMENT -> {
                    builder.setLayoutStrategy(DrawIoXmlBuilder.LayoutStrategy.HIERARCHICAL);
                    parsePlantUmlComponent(plantUmlCode, builder);
                }
                case USE_CASE -> {
                    builder.setLayoutStrategy(DrawIoXmlBuilder.LayoutStrategy.HIERARCHICAL);
                    parsePlantUmlUseCase(plantUmlCode, builder);
                }
                default -> {
                    builder.setLayoutStrategy(DrawIoXmlBuilder.LayoutStrategy.GRID);
                    parseGenericDiagram(plantUmlCode, builder);
                }
            }
        } catch (Exception e) {
            throw new RuntimeException(
                    "Failed to parse " + domainType + " PlantUML code: " + e.getMessage(), e);
        }

        logger.info("Draw.io XML built: {} nodes, {} edges", builder.getNodeCount(), builder.getEdgeCount());
        if (builder.getNodeCount() == 0) {
            logger.warn("No nodes parsed from {} PlantUML diagram — XML will be structurally valid but empty", domainType);
        }
        return builder.build();
    }
