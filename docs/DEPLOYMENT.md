...
**Confirmed empirically against a live Render deploy (Free instance, 512MB).**

- `NaturalLanguageParser` (`src/main/java/.../service/generation/parser/NaturalLanguageParser.java`) previously eagerly loaded a Stanford CoreNLP pipeline in `@PostConstruct` with annotators `tokenize,ssplit,pos,lemma,ner,depparse` — this loaded the POS tagger, NER models, and a full dependency parser into memory **at application startup**, before the app could serve any request.
- First live deploy attempt (Free plan, 512MB, `JAVA_TOOL_OPTIONS=-Xmx350m`) failed exactly as predicted: Spring context, Hibernate/Flyway, and DB connection all started successfully, then the process was OOM-killed by the OS while loading the NER classifier during pipeline construction — `==> Out of memory (used over 512Mi)`. This confirms the ~1–1.5GB pipeline footprint is a hard blocker for the Free/Starter tiers *at startup*, independent of `-Xmx` tuning (the container's total memory ceiling, not just JVM heap, was exceeded).
- **Fix applied**: `NaturalLanguageParser.init()` was changed from `@PostConstruct` to a private, synchronized lazy-init method (`pipeline()`), called from `parse()`/`parseToNlpResult()` on first actual use instead of at boot. This lets the application start and serve all non-NLP-dependent endpoints (auth, dashboard, editor, sharing, collaboration, repository import, AI-assisted editing via the LLM path) within 512MB. All 47 existing `NaturalLanguageParserTest` cases pass unchanged (test now omits the removed explicit `init()` call and relies on lazy init).
- **Residual risk, not eliminated**: this only defers the memory pressure — it does not reduce it. The first request that actually exercises the natural-language input path (`InputType.NATURAL_LANGUAGE`, i.e. free-text diagram generation, not XML/repo-URL input) will still attempt to load the ~1–1.5GB pipeline on a 512MB instance and will likely still OOM at that point, just later (mid-request instead of at boot). A Standard-tier plan (2GB RAM) or higher remains the recommended fix if the natural-language input path needs to be reliably supported in production; the lazy-load change only buys a working deployment for the rest of the app's features on the Free tier.
